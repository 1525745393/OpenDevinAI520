...
patterns:
  title_season:
    ...
    - pattern: "^(.+?)\\s+(?:Season|Saison)\\s*(\\d+)"
      description: "西式季数格式: Season 1"
    - pattern: "^(.+?)\\s+第(\\d+)季"
      description: "中文季数格式: 第1季"
  episode:
    - pattern: "E(\\d{1,3})"
      description: "E格式集数: E01"
    - pattern: "EP(\\d{1,3})"
      description: "EP格式集数: EP01"
    - pattern: "^第?(\\d{1,3})[集话]$"
      description: "方括号集数: [01]"
...
class TVShowProcessor:
    ...
    def process(self, preview: bool = None):
    ...
    def _collect_files(self, root, processed):
        file_list = []
        for dirpath, dirs, files in os.walk(root):
            for f in files:
                fp = os.path.join(dirpath, f)
                if self._is_valid_video(fp) and fp not in processed:
                    file_list.append(fp)
        return file_list

    def _scan_conflicts(self, all_files, preview):
        conflicts = {}
        for filepath in all_files:
    ...
    def _build_output_path(self, info):
    ...
    def _process_single_file(self, filepath, preview):
    ...
    def _move_file_with_retry(self, src, info, preview, meta_files):
        new_path = self._build_output_path(info)
    ...
    def _move_metadata_file(self, src_meta, dest_meta, preview, info=None):
        if preview:
        ...
        try:
            ...
            if src_meta.lower().endswith('.nfo') and info is not None:
                self._update_nfo_metadata(dest_meta, info)
        except Exception as e:
            self.logger.error(f" 元数据移动失败: {src_meta} - {e}")

    def _update_nfo_metadata(self, dest_meta, info):
        pass
    ...
    def _extract_episode(self, basename):
    ...
    def _extract_from_directory(self, dirname):
    ...
    def _extract_from_filename(self, basename):
        for pattern in self.config['patterns']['title_season']:
    ...
    def _smart_supplement(self, filepath, basename, dirname):
    ...
    def _get_associated_metadata(self, filepath):
    ...
    def _parse_season_number(self, s):
        cn_numbers = {
            '一': 1, '二': 2, '三': 3, '四': 4, '五': 5,
    ...
    def _parse_episode_number(self, s):
        cn_numbers = {
            '一': 1, '二': 2, '三': 3, '四': 4, '五': 5,
            '六': 6, '七': 7, '八': 8, '九': 9, '十': 10,
            '十一': 11, '十二': 12, '十三': 13, '十四': 14,
    ...
    def _record_undo_action(self, original, new_path):
    ...
    def _generate_tree_view(self, root_path, max_depth=3):
    ...
    def _should_skip_dir(self, path):
    ...
    def _show_results(self, preview):
        print(f"\n{Fore.CYAN}处理结果汇总:")
